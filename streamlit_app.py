...
- Sidebar navigation (tabs moved to sidebar)
- Trip Planner is the first page and its last computed route persists
- Added reverse geocoding (Nominatim) to show addresses for places and map clicks
- Save / load trip templates per user
- GPX export for planned routes
- "More features": route rounding, visit durations, and quick shareable read-only export token (local)
- New inline SVG logo (meaningful, compact) used in-app; also generated a raster logo externally
- Robust error handling and clearer UI flow
- Keeps black theme, centered logo, and per-user persistence
"""
...
if "page" not in st.session_state:
    st.session_state["page"] = "Trip Planner"  # Trip Planner first
if "last_route" not in st.session_state:
    st.session_state["last_route"] = None  # persist last computed route
if "trip_templates" not in st.session_state:
    st.session_state["trip_templates"] = {}  # per-user templates loaded on login

USERS_FILE = os.path.join(USER_DATA_DIR, "users.json")

# -------------------------
# New meaningful inline SVG logo (compact, modern)
# -------------------------
APP_SVG_LOGO = """
...
# -------------------------
# Reverse geocoding (Nominatim) - human-readable addresses
# -------------------------
...
def export_gpx(coords: List[Tuple[float, float]], name: str = "route"):
    ...
    for lat, lon in coords:
        trkpt = ET.SubElement(trkseg, "trkpt", attrib={"lat": f"{lat:.6f}", "lon": f"{lon:.6f}"})
        ...
        if st.button("Login", key="btn_login"):
            ...
            if token:
                ...
                if info and info.get("username") == username:
                    ...
                    # load trip templates if present
...
def page_dashboard():
    ...
    with c1:
        if st.button("Add sample place (dashboard)"):
            add_place("Sample Place", 0.3476, 32.5825, "Sample", category="Attractions", favorite=False)
...
def page_trip_planner():
    ...
    # Show last computed route if present (persisted)
    ...
    with col_t2:
        ...
        if st.button("Save template"):
            if new_template_name:
                # save selected indices as template
    ...
    if st.button("Compute optimized route"):
        ...
        # add visit durations to total_min
        ...
        # Save a quick share token (local)
        ...
        st.markdown(f"Shareable token (local): **{token}** — use this to load the route on this instance")
...
def run():
    ...
    # ensure user's places and templates loaded
