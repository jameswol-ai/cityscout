...
def build_ai_context(places: list[dict[str, Any]], params: SiteParameters) -> dict[str, Any]:
    safe_places = [p for p in places if isinstance(p, dict)]
    ...
    return {
        "places": _place_context(safe_places),
        "place_count": len(safe_places),
        "favorite_count": sum(bool(p.get("favorite")) for p in safe_places),
        "category_counts": counts,
        "spatial_summary": spatial_summary(safe_places),
...
def find_nearest_places(places: list[dict[str, Any]], origin: dict[str, Any], limit: int = 5) -> list[dict[str, Any]]:
    ...
    origin_candidates = _mapped_places([origin])
    if not origin_candidates:
        return []
    origin_safe = origin_candidates[0]
    try:
    ...
    ranked = []
    for place in mapped:
        if place is origin or (place.get("name") == origin_safe.get("name") and place.get("latitude") == origin_safe.get("latitude") and place.get("longitude") == origin_safe.get("longitude")):
            continue
...
def build_quick_itinerary(places: list[dict[str, Any]], start_name: str | None = None, limit: int = 6) -> dict[str, Any]:
    ...
    try:
        stop_limit = max(2, min(20, int(limit)))
    except (TypeError, ValueError):
        stop_limit = 6
    ...
    return {
        "stops": [{"order": i + 1, "name": p.get("name", "Unnamed"), "category": p.get("category", "Other"), "cost_tier": p.get("cost_tier", "$$")} for i, p in enumerate(order)],
        "total_km": round(total_km, 2),
        "estimated_activity_cost": round(costs.get("activity_cost", 0.0), 2),
    }
...
def _intent(text: str) -> str:
    ...
    if re.search(r"\b(report|intelligence|overview|summary|dashboard)\b", value): return "report"
    return "summary"
...
def run_agent_task(question: str, places: list[dict[str, Any]], context: dict[str, Any]) -> dict[str, Any]:
    ...
    if intent == "itinerary":
        result = build_quick_itinerary(places, limit=min(8, max(2, len(_mapped_places(places)))))
    ...
    if intent == "rank":
        return {"intent": intent, "title": "Top Saved Places", "message": "Ranked using favorites, description richness, and cost accessibility.", "data": rank_saved_places(places)[:10]}
    if intent == "report":
...
def _local_response(mode: str, question: str, context: dict[str, Any]) -> str:
    ...
    if mode == "Place Recommendations":
        favorites = [p.get("name", "Unnamed") for p in places if p.get("favorite")]
...
def _init_ai_state() -> None:
    if "ai_cityscout_history" not in st.session_state or not isinstance(st.session_state.ai_cityscout_history, list):
        st.session_state.ai_cityscout_history = []
    if len(st.session_state.ai_cityscout_history) > MAX_HISTORY_TURNS * 2:
        st.session_state.ai_cityscout_history = st.session_state.ai_cityscout_history[-MAX_HISTORY_TURNS * 2:]
...
def page_ai_cityscout() -> None:
    ...
    question = st.text_area("Ask CityScout", placeholder="Plan a one-day tour from my favorite place, prioritize attractions and food, and minimize backtracking.", height=100, key="ai_question")
    with st.expander("Concept site parameters", expanded=False):
        a, b, c = st.columns(3)
        site_area = a.number_input("Site area (m²)", min_value=100.0, value=1000.0, step=50.0, key="ai_site_area")
        coverage = b.slider("Site coverage (%)", 5, 95, 40, key="ai_coverage")
        floors = c.number_input("Floors", min_value=1, max_value=100, value=4, step=1, key="ai_floors")
        d, e, f = st.columns(3)
        floor_height = d.number_input("Floor height (m)", min_value=2.2, max_value=8.0, value=3.2, step=0.1, key="ai_floor_height")
        green_ratio = e.slider("Green ratio (%)", 0, 90, 25, key="ai_green_ratio")
        parking = f.number_input("Parking / 100 m²", min_value=0.0, value=1.0, step=0.1, key="ai_parking")
    ...
    if mode == "Agent Mode":
        ...
        if st.button("Run Agent", type="primary", use_container_width=True, key="ai_run_agent"):
            if not question.strip():
                ...
                st.session_state.ai_agent_result = run_agent_task(question.strip(), places, context)
        result = st.session_state.get("ai_agent_result")
        if result:
            st.markdown(f"#### {result.get('title', 'Agent result')}")
            ...
            if isinstance(data, list):
                ...
                st.json(data)
            if result.get("intent") == "report":
                st.download_button("Download agent report", json.dumps(data, indent=2, default=str), "cityscout_agent_report.json", "application/json", key="ai_download_agent_report")
    ...
    if tool == "Find Nearest Places":
        ...
        if names:
            origin_name = st.selectbox("From place", names, key="ai_nearest_origin")
            origin = next(p for p in _mapped_places(places) if p.get("name") == origin_name)
        ...
        categories = ["All"] + sorted({p.get("category", "Other") for p in places if isinstance(p, dict)})
        category = st.selectbox("Category", categories, key="ai_rank_category")
        ...
        names = [p.get("name", "Unnamed") for p in _mapped_places(places)]
        start = st.selectbox("Starting place", ["Automatic"] + names, key="ai_itinerary_start")
        max_limit = min(10, max(2, len(names)))
        limit = st.slider("Maximum stops", 2, max_limit, min(6, max_limit), key="ai_itinerary_limit") if names else 2
        if st.button("Build itinerary", key="ai_itinerary"):
            ...
            c1, c2 = st.columns(2)
            c1.metric("Estimated distance", f"{result['total_km']:.2f} km")
            c2.metric("Estimated activity cost", f"${result['estimated_activity_cost']:.2f}")
        ...
        report = {"summary": {"places": len(places), "favorites": context["favorite_count"], "categories": context["category_counts"]}, "spatial": context["spatial_summary"], "site": context["site_analysis"], "top_places": rank_saved_places(places)[:10]}
        st.json(report)
        st.download_button("Download city intelligence report", json.dumps(report, indent=2, default=str), "cityscout_intelligence_report.json", "application/json", key="ai_download_report")
